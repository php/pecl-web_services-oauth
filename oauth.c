...
#define SO_ADD_SIG(f, b) add_arg_for_req(f,OAUTH_PARAM_SIGNATURE, b TSRMLS_CC); efree(b);
        ...
        if(path) {
            ...
            if(post_args!=NULL) {
                for(zend_hash_internal_pointer_reset(post_args);zend_hash_get_current_key_ex(post_args,&post_cur_key,&post_cur_key_len,&post_num_key,0,NULL)!=HASH_KEY_NON_EXISTANT;zend_hash_move_forward(post_args)) {
                    ...
                    param_value = oauth_url_encode(Z_STRVAL_PP(current_arg_val));
...
SO_METHOD(fetch) {
    ...
    zval **token = NULL,*zret = NULL,**cs,*request_args = NULL,*ts = NULL,**token_secret = NULL,**p_current_req_val;
